...
repfigure contains simple utilites for generating visual representations of scientific reproduction and replication efforts.
This package is a companion to an article [ref] that defines these efforts and explains why such visualizations are helpful.
In this vignette, we will demonstrate how to use the functions in this package and recreate the main figure in [ref].
...
hide_stages = NULL
sci_figure(exps, rmk = T)	
```

To produce this figure, we first initialize a data frame using init_experiments, passing the parameter 2 to indicate that the
...
sci_figure(exps, hide_stages <- c("population", "hypothesis"), rmk = T)
```
...
```{r, fig.width = 9, fig.height = 7}
reproduce_figure(rmk=T)
```

```{r, fig.width = 9, fig.height = 7}
replicate_figure(rmk=T)
```

## Recreating Figure 1

The following code snippet recreates the columns in Figure 1 of [ref]:
